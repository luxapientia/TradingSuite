...
class BacktestRunner:
    ...
    async def fetch_price_data(self, symbol: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"Raw data columns for {symbol}: {df.columns.tolist()}")
            logger.info(f"Raw data shape for {symbol}: {df.shape}")
            ...
            logger.info(f"Fetched {len(df)} records for {symbol} from {df.index[0]} to {df.index[-1]}")
...
async def main():
    ...
    # Define date range (shorter for initial testing with mock data)
